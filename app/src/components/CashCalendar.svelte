  ...
  let calendarSimulation = $derived.by(() => {
    ...
    function projectMonth(startBalance, year, month) {
      ...
      for (let d = 1; d <= totalDays; d++) {
        ...
        if (appState.salaryConfig && d === (appState.salaryConfig.JourDePaie || 27)) {
          const sourceNodes = appState.flowNodes.filter(n => n.Type === 'Source' && n.Account_ID === acc.ID);
          if (sourceNodes.length > 0) {
            salaryReceived = parseFloat(appState.salaryConfig.MontantNet || 0);
              ...
              if (srcNode && srcNode.Account_ID === acc.ID) {
                if (c.TypeRegle === 'FIXED') {
                  ...
                  if (srcNode.Type === 'Source') {
                    ...
                    const remaining = Math.max(0, appState.salaryConfig.MontantNet - totalFixed);
              ...
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                ...
                <div style="display: flex; gap: 4px;">
                  {#if dayData?.salaryReceived > 0}
                    <div style="width: 8px; height: 8px; background-color: var(--sap-success-color); border-radius: 50%;" title="Salaire : +{appState.formatCurrency(dayData.salaryReceived)}"></div>
                  {/if}
                  {#if dayData?.debitsSum > 0}
                    <div style="width: 8px; height: 8px; background-color: var(--sap-error-color); border-radius: 50%;" title="{dayData.debits.length} prélèvement(s) : -{appState.formatCurrency(dayData.debitsSum)}"></div>
                  {/if}
                </div>
              </div>
